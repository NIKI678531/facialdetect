...
def process_row(row):
    try:
        ...
        # 计算embedding
        image_emb = compute_face_embedding(url)

        # 使用锁来保证文件写入的线程安全
        with file_lock:
            with open('/data/' + output_csv_path, 'a', newline='') as f:
                writer = csv.writer(f)
...
def process_csv_and_save_embeddings(input_csv_path, output_csv_path):
    ...
    # 创建输出CSV文件并写入表头
    with open('/data/' + output_csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['user_id', 'url', 'face'])

    # 设置线程池的最大线程数
    max_workers = 7  # 可以根据需要调整
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # 提交任务到线程池
        for _, row in df.iloc[25436+249259:].iterrows():
...
if __name__ == '__main__':
    ...
    output_csv_path = 'face_embeddings_output.csv'
