...
model, preprocess = clip_model.load_mobileclip_model()
...
def process_row(row):
    try:
        ...
        # 计算embedding
        image_emb = compute_person_embedding(url)
...
def process_csv_and_save_embeddings(input_csv_path, output_csv_path):
    ...
    with open(output_csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['user_id', 'url', 'person'])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # 提交任务到线程池
        for _, row in df.iloc[800000:].iterrows():
...
if __name__ == '__main__':
    ...
    output_csv_path = 'person_embeddings_output.csv'
